...
%  Display the shape.
%
  col = rand(face_num,1); % col必须为列向量
  handle = patch ( 'Vertices', node_xyz', 'Faces', face_node','FaceVertexCData',col,'FaceColor','flat' );
%     set ( handle, 'FaceColor', [1,1,1], 'EdgeColor', 'Black' );
...
%
  ...
  vertex_normal = zeros ( order_max, face_num ); %  一个面所有顶点法线在一列
%
...
%
  persistent lenc; % persistent变量对于声明它的函数来说是局部的，
                   % 但是当退出该函数时，该变量仍然保存在内存中，数值并不变
  persistent next;
  
  tab = char ( 9 ); % 制表符的ASCLL码
%
...
%
  while ( 1 )
    ...
    if ( lenc < ilo ) % 定位从右至左的第一非空字符，如果在首的左侧，意味着文本无单词
      word = ' ';
      done = 1;
      next = lenc + 1;
      return
...
function  detd = Plane_equation( node_xyz,face_node,face_num )
detd=zeros(face_num,4);
for i=1:face_num
    A_PLANE=face_node(i,1:end-1);
    A1=node_xyz(A_PLANE(1),:); B2=node_xyz(A_PLANE(2),:); C3=node_xyz(A_PLANE(3),:);
    A=B2-A1;
    B=C3-A1;
    cr=cross(A,B);
    a=cr(1); b=cr(2); c=cr(3);
    d=-(a*A1(1)+b*A1(2)+c*A1(3));
    detd(i,:)=-[a,b,c,d];
end
end
function  Center_xyz  = Geometric_center_point( node_xyz,face_node,face_order,face_num )
Center_xyz=zeros(face_num,3);
for i=1:face_num
    num=face_order(i,1);
